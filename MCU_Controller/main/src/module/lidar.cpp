...
void detectTask(void *pvParameter)
{
    lidar *sensor = static_cast<lidar *>(pvParameter);
    while (1)
    {
        vTaskDelay(pdMS_TO_TICKS(200));
        xSemaphoreTake(sensor->detectTaskSemaphore, portMAX_DELAY);
        ESP_LOGI(TAG, "Detecting...");
        if (sensor->detect()) sensor->motion->stop();
        else sensor->motion->resume();
        ESP_LOGI(TAG, "detectTaskWatermark: %d", uxTaskGetStackHighWaterMark(NULL));
        xSemaphoreGive(sensor->detectTaskSemaphore);
    }
}

lidar::lidar(int diam_balise, uart_port_t uart_num, uint8_t rx_pin, Motion *motion)
{
    this->diam_balise = diam_balise; // pour déterminer les clusters de points
    this->number_of_beacons = 0;
    for (int i = 0; i < NUMBER_OF_BEACONS_MAX; i++)
    {
        balise_position[i] = {0, 0, 0};
    }
    
    _ld19p = new ld19p(uart_num, rx_pin);
    this->motion = motion;

    detectTaskSemaphore = xSemaphoreCreateBinary();

    xTaskCreate(detectTask, "ledc_stepperStopTask", 8192*4, (void *)this, PRIORITY_DETECT_TASK, &detectTaskHandle);
}
...
int lidar::detect_object(LidarPoint_t raw_data[], int data_size, detected_object_t objects_detected[550], position_t robot_pos, int largeur_objet_min, int largeur_objet_max, int zone_in, int zone_out, int delta_zone)
{
    ...
    for (int i = 0; i < data_size; i++)
    {
        ...
        if (abs(raw_data[i].distance - raw_data[next].distance) < diam_balise)
        {
        ...
        else if (is_in_zone(object_abs_pos, zone_in, delta_zone)
            && !is_in_zone(object_abs_pos, zone_out, delta_zone))
        {
            ...
            if (largeur_objet_rad >= largeur_objet_min && largeur_objet_rad <= largeur_objet_max && number_of_detected_obj < 550)
            {
                objects_detected[number_of_detected_obj++] = {{ robot_pos.x + distance_objet*cos(deg_to_rad(robot_pos.r) + angle_objet_rad),
                                                                ...
                                                                0}, 
                                                                distance_objet, angle_objet_rad, largeur_objet_rad};
            }
        }
        else 
        {
...
bool lidar::set_number_of_beacons(int number_of_beacons)
{
    if (number_of_beacons <= NUMBER_OF_BEACONS_MAX) {
        this->number_of_beacons = number_of_beacons;
        return true;
    }
    return false;
}
...
void lidar::init_balise_pos(position_t robot_pos, int number_of_attempts_max)
{
    ...
    int data_size = _ld19p->get_full_tour(raw_data, 550);
    ...
    do
    {
        ...
        if (number_of_detected_obj == number_of_beacons)
        {
            continue;
        }
        // delay de 0.1s
    ...
    if (number_of_detected_obj == number_of_beacons) 
    {
        ESP_LOGI(TAG, "Les balises %d ont été détectées avec succès", this->number_of_beacons);
    } 
    else 
    {
        ESP_LOGE(TAG, "Initialisation des positions des balises : %d balises détectées au lieu de %d", number_of_detected_obj, this->number_of_beacons);
    }
    for (int i = 0; i < min(number_of_detected_obj, number_of_beacons); i++)
    {
        this->balise_position[i] = objects_detected[i].pos;
    }
}

void lidar::startDetection()
{
    xSemaphoreGive(detectTaskSemaphore);
    ESP_LOGI(TAG, "Detection started");
}

void lidar::stopDetection()
{
    xSemaphoreTake(detectTaskSemaphore, portMAX_DELAY);
    motion->resume();
...
bool lidar::detect()
{
    ...
    position_t delta = motion->relative_pos_futur();
    ...
    if (futur_loc.distance == 0) return false;
    int nb_pt = _ld19p->get_full_tour(points, 550);
    
    int current_cluster_size = 0;
    bool cluster_found = false;
    for (int i = 0; i < nb_pt + MIN_CLUSTER_SIZE; ++i) {
        if (pointIsObstacle(points[i%nb_pt], futur_loc)) ++current_cluster_size;
        else current_cluster_size = 0;
...
bool lidar::pointIsObstacle(LidarPoint_t point, LidarPoint_t futur_pos) // à optimiser avec un rectangle
{
...
{
    LidarPoint_t raw_data[550];
    int data_size = this->_ld19p->get_full_tour(raw_data, 550);
    ...
    {
        position_t real_position = balise_position[i];
        int j_min = 0;
